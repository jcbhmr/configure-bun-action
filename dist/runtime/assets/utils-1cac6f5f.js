import path$3, { dirname, join } from 'node:path';
...
import { createWriteStream, readFileSync, createReadStream, promises, existsSync } from 'node:fs';
import { setTimeout as setTimeout$1 } from 'node:timers/promises';
import require$$0$3 from 'stream';
import { debuglog } from 'node:util';
import { pipeline } from 'node:stream/promises';
import { readFile, chmod, stat, rm } from 'node:fs/promises';
...
identity$u.ALIAS = ALIAS;
identity$u.DOC = DOC;
identity$u.MAP = MAP;
identity$u.NODE_TYPE = NODE_TYPE;
identity$u.PAIR = PAIR;
identity$u.SCALAR = SCALAR$1;
identity$u.SEQ = SEQ;
identity$u.hasAnchor = hasAnchor;
identity$u.isAlias = isAlias;
identity$u.isCollection = isCollection$1;
identity$u.isDocument = isDocument;
identity$u.isMap = isMap;
identity$u.isNode = isNode;
identity$u.isPair = isPair;
identity$u.isScalar = isScalar$1;
identity$u.isSeq = isSeq;

var visit$6 = {};

var identity$t = identity$u;
...
function visit_(key, node, visitor, path) {
    ...
    if (identity$t.isNode(ctrl) || identity$t.isPair(ctrl)) {
        replaceNode(key, path, ctrl);
        return visit_(key, ctrl, visitor, path);
    }
    if (typeof ctrl !== 'symbol') {
        if (identity$t.isCollection(node)) {
        ...
        else if (identity$t.isPair(node)) {
            path = Object.freeze(path.concat(node));
...
function callVisitor(key, node, visitor, path) {
    ...
    if (identity$t.isMap(node))
        return visitor.Map?.(key, node, path);
    if (identity$t.isSeq(node))
        return visitor.Seq?.(key, node, path);
    if (identity$t.isPair(node))
        return visitor.Pair?.(key, node, path);
    if (identity$t.isScalar(node))
        return visitor.Scalar?.(key, node, path);
    if (identity$t.isAlias(node))
        return visitor.Alias?.(key, node, path);
    return undefined;
}
function replaceNode(key, path, node) {
    const parent = path[path.length - 1];
    if (identity$t.isCollection(parent)) {
        parent.items[key] = node;
    }
    else if (identity$t.isPair(parent)) {
        if (key === 'key')
            parent.key = node;
        else
            parent.value = node;
    }
    else if (identity$t.isDocument(parent)) {
        parent.contents = node;
    }
    else {
        const pt = identity$t.isAlias(parent) ? 'alias' : 'scalar';
        throw new Error(`Cannot replace node with ${pt} parent`);
    }
}

visit$6.visit = visit$5;
visit$6.visitAsync = visitAsync;

var identity$s = identity$u;
var visit$4 = visit$6;
...
class Directives {
    ...
    toString(doc) {
        ...
        let tagNames;
        if (doc && tagEntries.length > 0 && identity$s.isNode(doc.contents)) {
            const tags = {};
            visit$4.visit(doc.contents, (_key, node) => {
                if (identity$s.isNode(node) && node.tag)
                    tags[node.tag] = true;
            });
        ...
        setAnchors: () => {
            for (const source of aliasObjects) {
                ...
                if (typeof ref === 'object' &&
                    ref.anchor &&
                    (identity$r.isScalar(ref.node) || identity$r.isCollection(ref.node))) {
...
function toJS$7(value, arg, ctx) {
    ...
    if (value && typeof value.toJSON === 'function') {
        ...
        if (!ctx || !identity$q.hasAnchor(value))
...
var identity$p = identity$u;
var toJS$6 = toJS$8;

class NodeBase {
    constructor(type) {
        Object.defineProperty(this, identity$p.NODE_TYPE, { value: type });
    }
...
var visit$2 = visit$6;
var identity$o = identity$u;
var Node$2 = Node$3;
var toJS$5 = toJS$8;

let Alias$3 = class Alias extends Node$2.NodeBase {
    constructor(source) {
        super(identity$o.ALIAS);
        this.source = source;
...
function getAliasCount(doc, node, anchors) {
    if (identity$o.isAlias(node)) {
        const source = node.resolve(doc);
        const anchor = anchors && source && anchors.get(source);
        return anchor ? anchor.count * anchor.aliasCount : 0;
    }
    else if (identity$o.isCollection(node)) {
        let count = 0;
    ...
    else if (identity$o.isPair(node)) {
...
var identity$n = identity$u;
var Node$1 = Node$3;
var toJS$4 = toJS$8;

const isScalarValue = (value) => !value || (typeof value !== 'function' && typeof value !== 'object');
let Scalar$j = class Scalar extends Node$1.NodeBase {
    constructor(value) {
        super(identity$n.SCALAR);
        this.value = value;
    }
    toJSON(arg, ctx) {
...
function createNode$4(value, tagName, ctx) {
    if (identity$m.isDocument(value))
        value = value.contents;
    if (identity$m.isNode(value))
        return value;
    if (identity$m.isPair(value)) {
        const map = ctx.schema[identity$m.MAP].createNode?.(ctx.schema, null, ctx);
    ...
    if (aliasDuplicateObjects && value && typeof value === 'object') {
        ...
        if (ref) {
            ...
            return new Alias$2.Alias(ref.anchor);
        }
        else {
    ...
    if (!tagObj) {
        ...
        tagObj =
            value instanceof Map
                ? schema[identity$m.MAP]
                : Symbol.iterator in Object(value)
                    ? schema[identity$m.SEQ]
                    : schema[identity$m.MAP];
    }
    if (onTagObj) {
        onTagObj(tagObj);
...
let Collection$4 = class Collection extends Node.NodeBase {
    ...
    clone(schema) {
        ...
        if (schema)
            copy.schema = schema;
        copy.items = copy.items.map(it => identity$l.isNode(it) || identity$l.isPair(it) ? it.clone(schema) : it);
        if (this.range)
            copy.range = this.range.slice();
        return copy;
    }
    ...
    addIn(path, value) {
        if (isEmptyPath(path))
            ...
            if (identity$l.isCollection(node))
    ...
    getIn(path, keepScalar) {
        ...
        const node = this.get(key, true);
        if (rest.length === 0)
            return !keepScalar && identity$l.isScalar(node) ? node.value : node;
        else
            return identity$l.isCollection(node) ? node.getIn(rest, keepScalar) : undefined;
    }
    hasAllNullValues(allowScalar) {
        return this.items.every(node => {
            if (!identity$l.isPair(node))
                return false;
            const n = node.value;
            return (n == null ||
                (allowScalar &&
                    identity$l.isScalar(n) &&
                    n.value == null &&
                    !n.commentBefore &&
                    !n.comment &&
                    !n.tag));
    ...
    hasIn(path) {
        ...
        if (rest.length === 0)
            return this.has(key);
        const node = this.get(key, true);
        return identity$l.isCollection(node) ? node.hasIn(rest) : false;
    }
    /**
...
function stringifyProps(node, tagObj, { anchors: anchors$1$1, doc }) {
    if (!doc.directives)
        return '';
    const props = [];
    const anchor = (identity$k.isScalar(node) || identity$k.isCollection(node)) && node.anchor;
...
function stringify$6(item, ctx, onComment, onChompKeep) {
    if (identity$k.isPair(item))
        return item.toString(ctx, onComment, onChompKeep);
    if (identity$k.isAlias(item)) {
    ...
    let tagObj = undefined;
    const node = identity$k.isNode(item)
        ? item
...
function stringifyPair$1({ key, value }, ctx, onComment, onChompKeep) {
    ...
    let keyComment = (identity$j.isNode(key) && key.comment) || null;
    if (simpleKeys) {
        if (keyComment) {
            throw new Error('With simple keys, key nodes cannot have comments');
        }
        if (identity$j.isCollection(key)) {
    ...
    let explicitKey = !simpleKeys &&
        (!key ||
            ...
            identity$j.isCollection(key) ||
            (identity$j.isScalar(key)
    ...
    if (!explicitKey && !keyComment && identity$j.isScalar(value))
    ...
    else if (!explicitKey && identity$j.isCollection(value)) {
...
function addPairToJSMap$2(ctx, map, { key, value }) {
    if (ctx?.doc.schema.merge && isMergeKey(key)) {
        value = identity$i.isAlias(value) ? value.resolve(ctx.doc) : value;
        if (identity$i.isSeq(value))
...
function mergeToJSMap(ctx, map, value) {
    const source = ctx && identity$i.isAlias(value) ? value.resolve(ctx.doc) : value;
    if (!identity$i.isMap(source))
...
function stringifyKey(key, jsKey, ctx) {
    ...
    if (typeof jsKey !== 'object')
        return String(jsKey);
    if (identity$i.isNode(key) && ctx?.doc) {
...
let Pair$7 = class Pair {
    constructor(key, value = null) {
        Object.defineProperty(this, identity$h.NODE_TYPE, { value: identity$h.PAIR });
        this.key = key;
        this.value = value;
    }
    clone(schema) {
        let { key, value } = this;
        if (identity$h.isNode(key))
            key = key.clone(schema);
        if (identity$h.isNode(value))
...
function stringifyBlockCollection({ comment, items }, ctx, { blockItemPrefix, flowChars, itemIndent, onChompKeep, onComment }) {
    ...
    for (let i = 0; i < items.length; ++i) {
        ...
        else if (identity$g.isPair(item)) {
            const ik = identity$g.isNode(item.key) ? item.key : null;
            if (ik) {
...
function stringifyFlowCollection({ comment, items }, ctx, { flowChars, itemIndent, onComment }) {
    ...
    for (let i = 0; i < items.length; ++i) {
        ...
        if (identity$g.isNode(item)) {
            ...
            const iv = identity$g.isNode(item.value) ? item.value : null;
            if (iv) {
                if (iv.comment)
                    comment = iv.comment;
                if (iv.commentBefore)
    ...
    if (comment) {
        str += stringifyComment$1.lineComment(str, indent, commentString(comment));
        if (onComment)
            onComment();
    }
    return str;
...
var identity$f = identity$u;
var Pair$6 = Pair$8;
var Scalar$e = Scalar$k;

function findPair(items, key) {
    const k = identity$f.isScalar(key) ? key.value : key;
    for (const it of items) {
        if (identity$f.isPair(it)) {
            if (it.key === key || it.key === k)
                return it;
            if (identity$f.isScalar(it.key) && it.key.value === k)
                return it;
        }
    }
    return undefined;
}
let YAMLMap$7 = class YAMLMap extends Collection$2.Collection {
    static get tagName() {
        return 'tag:yaml.org,2002:map';
    }
    constructor(schema) {
        super(identity$f.MAP, schema);
        this.items = [];
    }
    /**
    ...
    add(pair, overwrite) {
        let _pair;
        if (identity$f.isPair(pair))
            _pair = pair;
        else if (!pair || typeof pair !== 'object' || !('key' in pair)) {
            // In TypeScript, this never happens.
            _pair = new Pair$6.Pair(pair, pair?.value);
        }
        else
            _pair = new Pair$6.Pair(pair.key, pair.value);
        ...
        if (prev) {
            ...
            if (identity$f.isScalar(prev.value) && Scalar$e.isScalarValue(_pair.value))
    ...
    get(key, keepScalar) {
        const it = findPair(this.items, key);
        const node = it?.value;
        return (!keepScalar && identity$f.isScalar(node) ? node.value : node) ?? undefined;
    }
    has(key) {
...
YAMLMap$8.YAMLMap = YAMLMap$7;
YAMLMap$8.findPair = findPair;

var identity$e = identity$u;
var YAMLMap$6 = YAMLMap$8;
...
const map$5 = {
    ...
    resolve(map, onError) {
        if (!identity$e.isMap(map))
            onError('Expected a mapping for this tag');
        return map;
    },
    createNode: (schema, obj, ctx) => YAMLMap$6.YAMLMap.from(schema, obj, ctx)
};
...
let YAMLSeq$6 = class YAMLSeq extends Collection$1.Collection {
    ...
    get(key, keepScalar) {
        ...
        return !keepScalar && identity$d.isScalar(it) ? it.value : it;
    }
    /**
     * Checks if the collection includes a value with the key `key`.
     *
    ...
    set(key, value) {
        ...
        if (identity$d.isScalar(prev) && Scalar$d.isScalarValue(value))
            prev.value = value;
        else
...
function asItemIndex(key) {
    let idx = identity$d.isScalar(key) ? key.value : key;
...
var identity$b = identity$u;
var Pair$5 = Pair$8;
var Scalar$7 = Scalar$k;
var YAMLSeq$4 = YAMLSeq$7;

function resolvePairs(seq, onError) {
    if (identity$b.isSeq(seq)) {
        for (let i = 0; i < seq.items.length; ++i) {
            let item = seq.items[i];
            if (identity$b.isPair(item))
                continue;
            else if (identity$b.isMap(item)) {
            ...
            seq.items[i] = identity$b.isPair(item) ? item : new Pair$5.Pair(item);
        }
    }
    else
        onError('Expected a sequence for this tag');
...
var identity$a = identity$u;
var toJS$1 = toJS$8;
var YAMLMap$5 = YAMLMap$8;
...
var identity$9 = identity$u;
var Pair$4 = Pair$8;
var YAMLMap$4 = YAMLMap$8;

class YAMLSet extends YAMLMap$4.YAMLMap {
    constructor(schema) {
        super(schema);
    ...
    get(key, keepPair) {
        ...
        return !keepPair && identity$9.isPair(pair)
            ? identity$9.isScalar(pair.key)
                ? pair.key.value
                : pair.key
            : pair;
    }
...
let Schema$1 = class Schema {
    constructor({ compat, customTags, merge, resolveKnownTags, schema, sortMapEntries, toStringDefaults }) {
        ...
        Object.defineProperty(this, identity$8.MAP, { value: map.map });
        Object.defineProperty(this, identity$8.SCALAR, { value: string.string });
        Object.defineProperty(this, identity$8.SEQ, { value: seq.seq });
        // Used by createMap()
        this.sortMapEntries =
...
function stringifyDocument$1(doc, options) {
    ...
    if (doc.contents) {
        if (identity$7.isNode(doc.contents)) {
...
let Document$3 = class Document {
    constructor(value, replacer, options) {
        ...
        this.warnings = [];
        Object.defineProperty(this, identity$6.NODE_TYPE, { value: identity$6.DOC });
        let _replacer = null;
    ...
    clone() {
        const copy = Object.create(Document.prototype, {
            [identity$6.NODE_TYPE]: { value: identity$6.DOC }
        });
        ...
        // @ts-expect-error We can't really know that this matches Contents.
        copy.contents = identity$6.isNode(this.contents)
    ...
    createAlias(node, name) {
        ...
        return new Alias$1.Alias(node.anchor);
    }
    createNode(value, replacer, options) {
        ...
        if (flow && identity$6.isCollection(node))
            node.flow = true;
        setAnchors();
        return node;
    }
     ...
     */
    get(key, keepScalar) {
        return identity$6.isCollection(this.contents)
    ...
    getIn(path, keepScalar) {
        if (Collection.isEmptyPath(path))
            return !keepScalar && identity$6.isScalar(this.contents)
                ? this.contents.value
                : this.contents;
        return identity$6.isCollection(this.contents)
     ...
     */
    has(key) {
        return identity$6.isCollection(this.contents) ? this.contents.has(key) : false;
    }
    /**
    ...
    hasIn(path) {
        ...
        return identity$6.isCollection(this.contents) ? this.contents.hasIn(path) : false;
    }
    /**
...
function assertCollection(contents) {
    if (identity$6.isCollection(contents))
        return true;
...
errors$4.YAMLError = YAMLError;
errors$4.YAMLParseError = YAMLParseError;
errors$4.YAMLWarning = YAMLWarning;
...
function mapIncludes(ctx, items, search) {
    ...
    const isEqual = typeof uniqueKeys === 'function'
        ? uniqueKeys
        : (a, b) => a === b ||
            (identity$5.isScalar(a) &&
                identity$5.isScalar(b) &&
                a.value === b.value &&
...
var identity$4 = identity$u;
var Pair$1 = Pair$8;
var YAMLMap$2 = YAMLMap$8;
...
function resolveFlowCollection$1({ composeNode, composeEmptyNode }, ctx, fc, onError, tag) {
    ...
    const NodeClass = (tag?.nodeClass ?? (isMap ? YAMLMap$2.YAMLMap : YAMLSeq$1.YAMLSeq));
...
var identity$3 = identity$u;
var Scalar$4 = Scalar$k;
var YAMLMap$1 = YAMLMap$8;
...
function composeCollection$1(CN, ctx, token, tagToken, onError) {
    ...
    if (!tagToken ||
        !tagName ||
        tagName === '!' ||
        (tagName === YAMLMap$1.YAMLMap.tagName && expType === 'map') ||
    ...
    const node = identity$3.isNode(res)
        ? res
...
function composeScalar$1(ctx, token, tagToken, onError) {
    ...
    const tag = tagToken && tagName
        ...
        : token.type === 'scalar'
            ...
            : ctx.schema[identity$2.SCALAR];
    let scalar;
    try {
        const res = tag.resolve(value, msg => onError(tagToken ?? token, 'TAG_RESOLVE_FAILED', msg), ctx.options);
        scalar = identity$2.isScalar(res) ? res : new Scalar$1.Scalar(res);
    }
    catch (error) {
...
function findScalarTagByName(schema, value, tagName, tagToken, onError) {
    if (tagName === '!')
        return schema[identity$2.SCALAR]; // non-specific tag
    const matchWithTest = [];
    ...
    return schema[identity$2.SCALAR];
}
function findScalarTagByTest({ directives, schema }, value, token, onError) {
    const tag = schema.tags.find(tag => tag.default && tag.test?.test(value)) || schema[identity$2.SCALAR];
    if (schema.compat) {
        const compat = schema.compat.find(tag => tag.default && tag.test?.test(value)) ??
            schema[identity$2.SCALAR];
...
publicApi$1.parseAllDocuments = parseAllDocuments;
publicApi$1.parseDocument = parseDocument;
publicApi$1.stringify = stringify;

var errors = errors$4;
var identity = identity$u;
var Pair = Pair$8;
var Scalar = Scalar$k;
var YAMLMap = YAMLMap$8;
...
async function main(stage) {
  ...
  const action = parse(await readFile(actionPath, "utf8"));
